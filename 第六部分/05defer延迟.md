...
- 返回值是切片、映射、函数、通道这些引用类型：defer 闭包可以修改返回值，因为待返回的临时“引用头”变量与局部变量 `x` 引用的是同一个底层数据，defer 闭包修改局部变量 `x` 的元素（其实修改的是底层数据）会影响最终的返回值
...
```go
// 切片、映射、函数、通道这些引用类型的返回值，可以在 defer 闭包中修改，影响最终的返回值
