...
`fmt.Print`、`fmt.Printf`、`fmt.Println` 是并发安全的，多个协程同时调用 `fmt.Println("hello")` 和 `fmt.Println("world")`，输出一定是完整的 `hello\n` 或 `world\n`，不会出现 `heworld\nllo\n` 这种交错。
...
| `%f`, `%e`, `%E`, `%g`, `%G` | 十进制浮点数。`%f` 小数形式，`%e`、`%E` 科学记数法，`%g`、`%G` 指数较大时用 `%e`/`%E`，否则用 `%f`（`%G` 使用大写字母 `E`） |
...
| `%%` | 百分号 |
| `%U` | 输出字符的 Unicode 码点，格式为 `U+` 后跟十六进制数字，不足 4 位补 0 |
...
需要注意：宽度和精度的单位是 Unicode 码点（rune），而非字节（byte），这意味着格式化时统计的“字符个数”是按 Unicode 字符算的。
