...
func main() {
 ...
 for i, v := range a {
  ...
  fmt.Printf("%v\t", v) // 输出 1 2 3 4 5  --不受影响，因为 range 是在数组的副本上迭代
 }
}
```
...
2. 变量 u 的地址与 users 切片中元素的地址都不相同，说明 u 是一个副本，如果结构体中含有较大 size 的字段（如示例中的 big 字段），复制副本的代价就比较大
...
range 迭代数组、切片时，循环次数在开始时就确定了。Go 在进入循环体之前，会把 range 后面的表达式求值一次，确定被遍历对象（一个副本）的长度 n，然后就固定循环 n 次。无论循环体中对原切片做了什么操作（追加或删除元素），都不会改变已经确定的循环次数和迭代结果。
...
应当避免在迭代切片时添加元素，因为可能导致切片扩容，使得迭代结果难以理解。
