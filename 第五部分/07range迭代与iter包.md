...
| 切片 | `for i, v := range s` | 元素的索引 `i`（从 0 开始）和值 `v`（副本） | 循环次数在迭代开始时就确定了，中途不会改变。注意：1. `v` 是元素的副本，修改它不影响原切片 2. `range s` 迭代的是 s 的“切片头”的副本，在循环中对 s 的修改（如增删元素）不会影响迭代结果 3. 对 `nil` 切片执行 range 是安全的，迭代次数为 0 |
| 映射 | `for k, v := range m` | 键 `k`，值 `v`（副本） | 迭代顺序不保证，且每次迭代都会随机化（Go 有意为之，避免程序依赖特定顺序）。注意：1. `v` 是值的副本，修改它不影响原映射 2. 虽然 `range m` 迭代的也是 m 的一个副本，但副本与 m 指向同一个底层数据，所以可以忽略副本行为带来的影响 3. 对 `nil` 映射执行 range 是安全的，迭代次数为 0 |
...
| 通道 | `for v := range c` | 接收到的值 `v` | `range` 会持续从通道接收数据，直到通道被显式关闭才退出循环，并在退出前接收完通道关闭前已缓冲的所有数据；若当前没有可接收的值则阻塞等待 |

在迭代之前，`range` 关键字会先对它后面的表达式求值（且仅求值一次），并把求值结果保存下来，之后就在这个保存的结果（副本）上迭代。这是 `range` 迭代的一个非常关键的行为。

| 类型 | range 求值后保存的副本 | 拷贝开销 |
| -- | -- | -- |
| 数组 `[N]T` | 整个数组（N 个元素） | 大 |
| 切片 `[]T` | 切片的“引用头”（data 指针+len+cap） | 小 |
| 字符串 `string` | 字符串头（str 指针+len） | 小 |
| 映射 `map` | 映射的“引用头” | 小 |
| 通道 `chan` | 通道的“引用头” | 小 |

数组是值类型，`range a` 会拷贝整个数组，然后在这个副本上迭代。对于大型数组，拷贝的开销很大，应使用 `range &a` 或 `range a[:]` 减少拷贝开销。
...
例外：当 `range` 的迭代对象是数组，且只有一个迭代变量，且 `len(a)` 是常量时，编译器会跳过求值/拷贝这一步，直接在原数组上迭代，自然也就没有拷贝开销。

```go
func main() {
 {
  a := [5]int{1, 2, 3, 4, 5}
  for i := range a { // 只有一个变量 -> 触发优化，不求值、不拷贝
   a[i] = a[i] * 2
   fmt.Printf("a[%v]=%v\t ", i, a[i]) // 2 4 6 8 10
  }
 }

 {
  a := [5]int{1, 2, 3, 4, 5}
  for i, v := range a { // 有两个变量 -> 拷贝整个数组
   a[i] = a[i] * 2
   fmt.Printf("a[%v]=%v\t ", i, v) // 1 2 3 4 5
  }
 }
}
```
