...
| 映射 | `for k, v := range m` | 键 `k`，值 `v`（副本） | 迭代顺序是随机的（Go 有意为之），但在迭代开始时就确定了。注意：1. `v` 是值的副本，修改它不影响原映射 2. 虽然 `range m` 迭代的也是 m 的一个副本，但副本与 m 共享同一个底层数据，且这个底层数据在 m 扩容时不会改变，所以可以忽略这一点 3. 对 `nil` 映射执行 range 是安全的，迭代次数为 0 |
