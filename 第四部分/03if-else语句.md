...
if 条件表达式1 { // 这个 { 不能换行
 // 条件1 为 true 时执行
} else if 条件表达式2 { // else 必须紧跟在 } 之后，不能换行，并且 { 不能换行
 // 条件2 为 true 时执行
} else {
 // 其它情况执行
}
...
func pow(x, n, lim float64) float64 {
 // v 仅在 if-else 代码块中有效
 if v := math.Pow(x, n); v < lim {  // Pow 返回 x**n，即 x 的 n 次幂
  return v
 } else {
  // %g 是浮点数的通用格式化动词，会根据指数大小自动选择 %f 或 %e，并省略不必要的尾随零
  fmt.Printf("%g >= %g\n", v, lim)
 }
...
注意：初始化语句中声明的变量可能遮蔽外层同名变量。

## 条件书写原则

if-else 条件的书写原则：

- 安全第一：`nil` 检查、数组越界检查、除零检查等防御性条件必须优先
