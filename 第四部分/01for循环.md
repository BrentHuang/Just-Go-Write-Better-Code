...
```go
for 初始化语句; 条件表达式; 后置语句 { // 这个 { 不能换行
    // 循环体
}
```

for 循环的初始化语句通常是一个短变量声明（`:=`），声明的变量仅在 for 循环中有效。当条件表达式为 false 时结束循环。
...
在嵌套循环中，`break` 和 `continue` 默认只作用于最内层循环。若需控制外层循环，可配合标签（label）使用，标签命名遵循 Go 的驼峰命名法，习惯上采用首字母大写（如 `RowLoop`）：

```go
func main() {
RowLoop:  // label 命名采用首字母大写的驼峰命名法。此处的 label 名称应体现外层循环的职责
 for i := range 10 {
  for j := range 5 {
   fmt.Printf("%d %d\n", i, j)
...
- 标签的作用域被限定在定义它的函数体中，无法跨函数使用。同一函数体中，标签名必须唯一；但标签不与变量、类型等其它标识符冲突，可以与其同名
